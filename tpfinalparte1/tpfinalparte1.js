...
let click;
let Imagenes = [];
let musicaIniciada = false; // Nueva variable para controlar la música
...
function draw() {
  ...
  if (pantalla === 0) {
    ...
    // Descomentar para ver el área del botón de música:
...
function mousePressed() {
  ...
  // Botón invisible de música en esquina superior derecha (pantalla 0)
  if (pantalla === 0) {
    if (mouseX > 540 && mouseX < 640 && mouseY > 0 && mouseY < 100) {
      if (musica.isPlaying()) {
        // Si está tocando, pausar la música
        musica.pause();
      } else {
        // Si está pausada o no ha empezado, reproducir
        if (!musicaIniciada) {
